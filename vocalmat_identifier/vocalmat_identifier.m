...
save_spectrogram_background = 1;

% -- local_median: option to use the local median method to detect noise
local_median = 1;

% -- save output option to save output files
save_output_files = 1;
...
% -- run through each segment, compute the spectrogram, and process its outputs
    ...
    % -- remove frequencies below 30kHz #modified by K. J. Scott for rats,
    % in order to minimize in our noisy recordings, modify as you see fit
    min_freq  = find(F>30000);
...
% -- for each connected component, get vocalization x-coordinates (time_vocal) and save frequency points (freq_vocal, y-coordinates)
    if k == 1
        ...
        if min(graindata_2(k).PixelList(:,1)) - max(time_vocal{id}) > max_interval %JACK NOTE - This may be where we are getting our double ups. Need to investigate.
