...
% -- Modified by K.J. Scott (Otago University, 2022) to recognise a Wright et al. (2010) classifiers
% -- for use with Rat USVs
% ----------------------------------------------------------------------------------------------
...
%-----------------------------------------------------------------------------------
% Call classifiers loosely based on the following paper:
% Wright JM, Gourdon JC, Clarke PB. Identification of multiple call categories within the rich repertoire
% of adult rat 50-kHz ultrasonic vocalizations: effects of amphetamine and social context. 
% Psychopharmacology (Berl). 2010 Jul;211(1):1-13. doi: 10.1007/s00213-010-1859-y.
%----------------------------------------------------------------------------------
...
CompTrill_count = sum(strcmp(T_classProb.DL_out,'CompTrill'));
DownRamp_count = sum(strcmp(T_classProb.DL_out,'DownRamp'));
Flat_count = sum(strcmp(T_classProb.DL_out,'Flat'));
Inverted_U_count = sum(strcmp(T_classProb.DL_out,'Inverted_U'));
Noise_count = sum(strcmp(T_classProb.DL_out,'Noise'));
Split_count = sum(strcmp(T_classProb.DL_out,'Split'));
...
Trill_count = sum(strcmp(T_classProb.DL_out,'Trill'));
UpRamp_count = sum(strcmp(T_classProb.DL_out,'UpRamp'));
noise_dist_count = sum(strcmp(T_classProb.DL_out,'Noise'));
...
if save_excel_file==1
    ...
    Noise_idx = strcmp(tabela(:,18),'Noise');
